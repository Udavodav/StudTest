...
class StoreController extends Controller
{
    public function __invoke(StoreRequest $request)
    {
        ...
        dd($data);
        $dataQuestion = $data;
        unset($dataQuestion['is_right']);
        unset($dataQuestion['textOp']);
        unset($dataQuestion['path_imageOp']);
        ...
        foreach ($data['textOp'] as $key => $value){
            $answer['question_id'] = $question['id'];
            $answer['text'] = $value;
            $answer['is_right'] = ((int)($data['is_right'][0]) - 1) == $key ? 1 : 0;
